...
public class IOUContract implements Contract {
    ...
    public void verify(LedgerTransaction tx) throws IllegalArgumentException {
        ...
        requireThat(check -> {
            ...
            check.using("The producer and the buyer cannot be the same entity.", producer != buyer);

            // Constraints on the signers.
            final List<PublicKey> signers = command.getSigners();
            check.using("There must be two signers.", signers.size() == 2);
            check.using("The producer and the buyer must be signers.", signers.containsAll(
